      ...
      <div class="flex flex-col flex-1 w-full">
        ...
        <main class="h-full overflow-y-auto">
          <div class="container px-6 mx-auto grid">
            <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">會員管理</h2>
            <h4 class="mb-4 text-lg font-semibold text-gray-600 dark:text-gray-300">
              <a href="forms.php">+新增會員資料</a>
            </h4>
            <!-- New Table -->
                ...
                <table class="w-full whitespace-no-wrap">
                  <thead>
                    <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400
                     dark:bg-gray-800">
                      <th class="px-4 py-3">學號/帳號</th>
                      <th class="px-4 py-3">密碼</th>
                      <th class="px-4 py-3">姓名</th>
                      <th class="px-4 py-3">系級</th>
                      <th class="px-4 py-3">身分</th>
                      <th class="px-4 py-3">權限</th>
                      <th class="px-4 py-3">編輯</th>
                    </tr>
                    <?php
                    $link = mysqli_connect('localhost', 'root', '12345678', 'fjuim');
                    if(isset($searchtxt)){
                        $sql = "select * from member1 where ID like '%". $searchtxt. "%'";
                    }
                    else{
                    ...
                    while($row = mysqli_fetch_assoc($result)){
                        echo "<tr><td>", $row['ID'], "</td><td>", $row['pword'], "</td><td>", $row['Name'], "</td><td>", $row['Grade'], "</td><td>", $row['Identity'],
                         "</td><td>", $row['Limits'], "</td><td><br><a href='update_mem.php?ID=", $row['ID'], "'>修改 / </a><a href='delete_mem.php?ID=",
                          $row['ID'], "'>刪除</a></tr>";
                    }
                    ?>
                  </thead>
